...
export default function Inflation() {
  ...
  const load = async () => { setStatus('FETCHING TR MACRO DATA...'); setError(false); try { const response = await fetch('/api/tr-macro', { cache: 'no-store' }); const snapshot = await response.json(); if (!response.ok) throw Error(snapshot.error); setData(snapshot); setStatus(`LIVE · ${new Date(snapshot.fetchedAt).toLocaleTimeString('en-US')}`); } catch { setData(); setError(true); setStatus('LIVE DATA UNAVAILABLE'); } };
  useEffect(() => { load(); }, []);
